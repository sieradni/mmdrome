...
**Status**: design approved direction. **Phase 0 LANDED** (`a9c463b`): `StreamSchedule.swift` + `StreamPolicy.swift` pure cores pinned in `StreamScheduleTests`/`StreamPolicyTests` — inert, the engine does not import them yet. Phases 1-4 (loader staging, engine staged scheduling, crossfade readiness, default flip) are NOT implemented; the anchor A15 in AGENTS.md records the current runtime truth (native = download-then-play, `loadAndStart` schedules only from the full-file completion).
