...
**Status**: design proposal, not implementation. Nothing here is committed; the anchor A15 in AGENTS.md records the current truth (native = download-then-play, `loadAndStart` schedules only from the full-file completion).
