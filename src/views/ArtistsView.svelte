        ...
        {#each selectedTracks as track, idx (track.trackId)}
          <div class="flex items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-surface-hover">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="truncate text-xs text-muted">{track.album} · {track.year ?? '—'} · {formatDuration(track.duration)}</p>
            </div>
            <div class="flex flex-shrink-0 items-center gap-0.5">
              {#each starSegments(getRating(track.trackId)) as seg, si}
                <svg class="h-3 w-3" viewBox="0 0 24 24">
                  {#if seg === 'half'}
                    <defs><linearGradient id="ar-{track.trackId}-{si}"><stop offset="50%" stop-color="#facc15" /><stop offset="50%" stop-color="transparent" /></linearGradient></defs>
                  {/if}
                  <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26Z" fill={seg === 'full' ? '#facc15' : seg === 'half' ? 'url(#ar-' + track.trackId + '-' + si + ')' : 'none'} stroke={seg === 'empty' ? '#555' : '#facc15'} stroke-width="1" />
                </svg>
              {/each}
            </div>
